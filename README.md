...
1. **Objective 1**: (Not implemented in provided files)
2. **Objective 2**: (Not implemented in provided files)
...
- `Student2.R`: Empty file.
- `Student3.R`: Corrupted file (Mac OS extended attributes).
- `Student4.R`: Corrupted file (Mac OS extended attributes).
- `_EDA.R`, `_Student1.R`, `_Student2.R`, `_rough.R`: Corrupted backup files (can be ignored or removed).
...
```r
install.packages(c("ggplot2", "dplyr", "tidyr", "ggridges", "GGally", "streamgraph", "fmsb"))
```
...
2. Run the R scripts in RStudio or R console. Each script assumes `df` is available in the environment.

3. For Objective 3: Execute `Student1.R` to analyze temporal effects.

4. For Objective 4: Execute `EDA.R` to analyze external delay factors.
